...
autcor <- function (y, lag = NULL, plot = TRUE, lag_axis = TRUE)
{
    ...
    if (plot == TRUE) {
        plot((0:lag), z[[1L]], type = "h", ylab = "Autocorrelation",
             xlab = "Lag")
        if (lag_axis == TRUE)
            abline(h = 0, lty = 1)
    }
...
#  n2p, n : definition    n=2**n2p >= ld+lag1 >= 2**(n2p-1)
    ...
    if(isw == 4) {
        fftcor.out  <- list(acov = acov, ccov12 = z[[3L]][1:lag1],
                            ccov21 = z[[2L]][1:lag1], acor = cna1,
                            ccor12 = z[[6L]], ccor21 = z[[5L]], mean = amean)
        if(plot == TRUE) {
            par(mfrow=c(2,1))
            plot((0:lag), z[[6L]], type = "h", xlab = "Lag",
                 ylab = "Crosscorrelation ccor12")
            if(lag_axis == TRUE)
                abline(h = 0, lty = 1)
            plot((0:lag), z[[5L]], type = "h", xlab = "Lag",
...
function (y, lag=NULL, window = "Akaike", log = FALSE, plot = TRUE)
{
    if (window != "Akaike" && window != "Hanning")
        stop(paste("Allowed windows are", '"Akaike"', "or", '"Hanning"',"."),
             sep="")
...
function (y, lag = NULL, window = "Akaike", plot = TRUE, plot.scale = FALSE) 
{
    ...
    mulspe.out <- list(spec = spec, stat = array(z[[3L]], dim = c(lag1, d)),
                       coh = coh)
...
function (y, lag = NULL, invar, outvar)
{
    ...
    sglfre.out <- list(inspec = z[[1L]], outspec = z[[2L]], cspec = z[[3L]],
                       qspec = z[[4L]], gain = z[[5L]], coh = z[[6L]],
                       freqr = z[[7L]], freqi = z[[8L]], errstat = z[[9L]],
                       phase = z[[10L]])
    return(sglfre.out)
}
...
fpeaut <- function (y, max.order = NULL)
{
    ...
    mo <- z[[9L]]
    fpeaut.out <- list(ordermin = mo, best.ar = arcoef[[mo]], sigma2m = z[[10L]],
                       fpemin = z[[7L]], rfpemin = z[[8L]], ofpe = z[[6L]],
                       arcoef = arcoef, sigma2 = z[[1L]], fpe = z[[2L]],
                       rfpe = z[[3L]], parcor = z[[4L]], chi2 = z[[5L]])
    return(fpeaut.out)
}
...
fpec <- function (y, max.order = NULL, control = NULL, manip = NULL)
{
 ...
 equal to d")
    ...
    fpec.out  <- list(cov = cov, fpec = z[[2L]], rfpec = z[[3L]], aic = z[[4L]],
                      ordermin = z[[5L]], fpecmin = z[[6L]], rfpecmin = z[[7L]],
                      aicmin = z[[8L]], perr=array(z[[9L]], dim = c(ncon,ncon)),
                      arcoef = arcoef)
    class(fpec.out) <- "fpec"
    return(fpec.out)
}
...
mulnos <- function (y, max.order = NULL, control = NULL, manip = NULL, h)
{
 ...
 equal to d")
    ...
    mulnos.out <- list(nperr = array(z[[1L]], dim = c(ip, ip)),
                       diffr = array(z[[2L]], dim = c(ip, ip, (h+1))),
          ...
          plot.scale = FALSE) 
{
    ...
    mulrsp.out <- list(rspec = array(z[[1L]], dim = c(d, d, (h+1))),
                       scoh = array(z[[2L]], dim = c(d, d, (h+1))))
...
optdes <- function (y, max.order = NULL, ns, q, r)
{
    ...
    optdes.out <- list(perr = z1$perr, trans = a, gamma = b,
                       gain = array(z[[1L]], dim = c(nman, order*ncon)))
    return(optdes.out)
}
...
function (y, max.order =NULL, ns, q, r, noise = NULL, len, plot = TRUE)
{
    ...
    if (plot == TRUE) {
        ...
        optsim.out <- list(trans = trans, gamma = gamma, gain = gain,
                           xmean = z[[3L]], ymean = z[[4L]], xvar = z[[9L]],
                           yvar = z[[10L]], x2sum = z[[5L]], y2sum = z[[6L]],
                           x2mean = z[[7L]], y2mean = z[[8L]])
    } else {
        optsim.out <- list(trans = trans, gamma = gamma, gain = gain,
                           convar = convar, manvar = manvar, xmean = z[[3L]],
                           ymean = z[[4L]], xvar = z[[9L]], yvar = z[[10L]],
                           x2sum = z[[5L]], y2sum = z[[6L]], x2mean = z[[7L]],
                           y2mean = z[[8L]])
    }
    return(optsim.out)
}
...
wnoise <- function (len, perr, plot = TRUE)
{
    ...
    if (ir != 1)
        wnoise <- array(z[[1L]], dim = c(ir, len))
    if (ir == 1)
        wnoise <- z[[1L]][1:len]
...
bispec <- function (y, lag = NULL, window = "Akaike", log = FALSE, plot = TRUE)
{
    ...
    bispec.out <- list(pspec = pspec, sig = z[[3L]],
                       cohe = array(z[[4L]], dim = c(lag1, lag1)),
                       breal = array(z[[5L]], dim = c(lag1, lag1)),
                       bimag = array(z[[6L]], dim = c(lag1, lag1)),
                       exval = z[[7L]])
    return(bispec.out)
}
...
#        abline(h = -1/sqrt(n), lty = 3)
    ...
    canarm.out <- list(arinit = -ar[1:mo], v = z1[[2L]][1:l1],
                       aic = z1[[3L]][1:l1], aicmin = z1[[4L]],
                       order.maice = mo, parcor = parcor, nc = nc, future = m1,
                       past = m2, cweight = cweight, canocoef = cR,
                       canocoef2 = Rsquar, chisquar = chisquar, ndf = ndf,
                       dic = dic, dicmin = z1[[16L]][1:nc],
                       order.dicmin = z1[[17L]][1:nc], arcoef = -z1[[19L]][1:k],
                       macoef = -z1[[21L]][1:l])
    return(canarm.out)
}
...
# the k-th component of y(i) is the j-th component of the original record z(i)
    ...
    canoca.out <- list(aic = z1[[2L]][1:(l+1)], aicmin = z1[[3L]],
                       order.maice = mo, v = array(z1[[5L]], dim = c(d, d)),
                       arcoef = arcoef, nc = nc, future = m1, past = m2,
                       cweight = cweight, canocoef = cR, canocoef2 = Rsquar,
                       chisquar = chisquar, ndf = ndf, dic = dic,
                       dicmin = z1[[16L]][1:nc], order.dicmin = z1[[17L]][1:nc],
                       matF = matF, vectH = z1[[20L]][1:k],
                       matG = g[(1:k),(1:d)], vectF = z1[[23L]][1:ivf])
    return(canoca.out)
}
...
covgen <- function (lag, f, gain, plot = TRUE)
{
    ...
    if (plot == TRUE) {
        plot((0:lag), z[[2L]], type = "h", ylab = "Auto Covariance Normalized",
             xlab = "Lag")
      abline(h = 0, lty = 1)
    }
...
# initial estimates of the free parameters in G
    ...
    markov.out <- list(id = z[[1L]], ir = z[[2L]], ij = z[[3L]], ik = z[[4L]],
                       grad = z[[6L]][1:ngr],
                       matFi = array(z[[7L]], dim = c(k, k)),
                       matF = array(z[[8L]], dim = c(k, k)),
                       matG = array(z[[9L]], dim = c(k, d)),
                       davvar = vd[(1:ngr),(1:ngr)], arcoef = arcoef,
                       impulse = array(au, dim = c(d, d, iqm-1)),
                       macoef = macoef, v = array(z[[15L]], dim = c(d, d)),
                       aic = z[[16L]])
    return(markov.out)
}
    ...
    if (plot == TRUE) {
        ...
        for(i in 1:ns) {
            plot(x, pspec[,i], type = "l",
                 main = paste("y(", z[[7L]][i], "),...,y(", z[[8L]][i], ")"),
                 xlab = "Frequency", ylab = "Power Spectrum")
        }
        par(oldpar)
        nonst.out <- list(ns = ns, arcoef = arcoef, v = z[[3L]], aic = z[[4L]],
                          daic21 = z[[5L]], daic = z[[6L]], init = z[[7L]],
                          end = z[[8L]])
    } else {
        nonst.out <- list(ns = ns, arcoef = arcoef, v = z[[3L]], aic = z[[4L]],
                          daic21 = z[[5L]], daic = z[[6L]], init = z[[7L]],
                          end = z[[8L]], pspec = pspec)
    }
...
simcon <- function (span, len, r, arcoef, impulse, v, weight)
{
    ...
    simcon.out <- list(gain = array(z[[3L]], dim = c(r, k*d)), ave = z[[4L]][1:d],
                       var = z[[5L]][1:d], std = z[[6L]][1:d],
                       bc = array(z[[1L]], dim = c(k*d, r)),
                       bd = array(z[[2L]], dim = c(k*d, d-r)))
    return(simcon.out)
}
...
thirmo <- function (y, lag = NULL, plot = TRUE)
{
    ...
    if (plot == TRUE) {
        plot((0:lag), z[[3L]], type = "h", ylab = "Auto Covariance Normalized",
             xlab = "Lag")
        abline(h = 0, lty = 1)
    }

    thirmo.out <- list(mean = z[[1L]], acov = z[[2L]], acor = z[[3L]],
                       tmomnt = tmomnt)
    return(thirmo.out)
}
...
blocar <- function (y, max.order = NULL, span, plot = TRUE)
{
    ...
    for(i in 1:ns)
        pacoef[[i]] <- z[[5L]][((i-1)*morder+1):(i*morder)]
    for(i in 1:ns)
        arcoef[[i]] <- z[[6L]][((i-1)*morder+1):(i*morder)]
    ...
    blocar.out <- list(mean = z[[1L]], var = z[[2L]], aic = aic,
...
# upper limit of the order of AR-model
    ...
    blomar.out <- list(mean = z[[1L]], var = z[[2L]], bweight = bweight,
...
#    cnst <- 0                         # constant value (mtype=6)
    ...
    if (plot == TRUE) {
        ...
        if (mtype == 1) {
            k <- lag
            k1 <- k + 1
            plot((0:k), z[[9L]][1:k1], ylim = c(0,40), type = "l", xlab = "Lag",
                 ylab = "AIC(m)-aicmin")
            abline(h=0, lty=1)
        } else {
            plot((0:nreg), z[[9L]], ylim = c(0,40), type = "l", xlab = "Lag",
                 ylab = "AIC(m)-aicmin")
            abline(h=0, lty=1)
        }
        ...
        if (mtype == 1) {
            ...
            for(i in 1:121)
                x[i] <- (i - 1) / 240
            plot(x, z[[29L]], type = "l", xlab = "Frequency",
    ...
    if (mtype == 1) {
        k <- lag
        k1 <- k + 1
	bsubst.out <- list(ymean = z[[1L]], yvar = z[[2L]], v = z[[7L]][1:k1],
                    aic = z[[8L]][1:k1], aicmin = z[[4L]], daic = z[[9L]][1:k1],
                    order.maice = mo, v.maice = z[[5L]], arcoef.maice = arcoefm,
                    v.bay = z[[11L]], aic.bay = z[[10L]], np.bay = z[[12L]],
                    arcoef.bay = z[[13L]][1:k], ind.c = z[[14L]][1:k],
                    parcor2 = z[[15L]][1:k], damp = z[[16L]][1:k1],
                    bweight = z[[17L]][1:k], parcor.bay = z[[18L]][1:k],
                    eicmin = z[[19L]], esum = z[[20L]][1:k1], npmean = z[[21L]],
                    npmean.nreg = z[[22L]], perr = perr, mean = z[[24L]],
                    var = z[[25L]], skew = z[[26L]], peak = z[[27L]],
                    peautcor = autcor1, pspec = z[[29]])
    }

    if (mtype==2 || mtype==3)
	bsubst.out <- list(ymean = z[[1L]], yvar = z[[2L]], v = z[[7L]],
                    aic = z[[8L]], aicmin = z[[4L]], daic = z[[9L]],
                    order.maice = mo, v.maice = z[[5L]], arcoef.maice = arcoefm,
                    v.bay = z[[11L]], aic.bay = z[[10L]], np.bay = z[[12L]],
                    arcoef.bay = z[[13L]], ind.c = z[[14L]], parcor2 = z[[15L]],
                    damp = z[[16L]], bweight = z[[17L]], parcor.bay = z[[18L]],
                    eicmin = z[[19L]], esum = z[[20L]], npmean = z[[21L]],
                    npmean.nreg = z[[22L]], perr = perr, mean = z[[24L]],
                    var = z[[25L]], skew = z[[26L]], peak = z[[27L]],
                    peautcor = autcor1)
...
exsar <- function (y, max.order = NULL, plot = FALSE)
{
    ...
    if (plot == TRUE) {
        plot((0:morder), z[[5L]], ylim = c(0,40), type = "l", xlab = "Lag",
              ylab = "AIC(m)-aicmin (Truncated at 40.0)")
        abline(h = 0, lty = 1)
     }

    m <- z[[6L]]
    exsar.out  <- list(mean = z[[1L]], var = z[[2L]], v = z[[3L]], aic = z[[4L]],
                       aicmin = z[[7L]], daic = z[[5L]], order.maice = m,
                       v.maice = z[[8L]], arcoef.maice = z[[9L]][1:m],
                       v.mle = z[[10L]], arcoef.mle = z[[11L]][1:m])
...
#    pspec <- array(z[[8L]], dim=c(121,ns))
    ...
    mlocar.out <- list(mean = z[[1L]], var = z[[2L]], ns = ns, order = z[[4L]],
                       arcoef = arcoef, v = z[[5L]], init = z[[6L]],
                       end = z[[7L]], pspec = pspec, npre = npre,
                       nnew = z[[10L]], order.mov = z[[11L]], v.mov = z[[12L]],
                       aic.mov = z[[13L]], order.const = order.const,
...
mlomar <- function (y, max.order = NULL, span, const = 0)
{
    ...
    arcoef <- list()
    for(i in 1:nns)
        arcoef[[i]] <- array(a[, , , i], dim = c(d, d, z[[9L]][i]))
    ...
    mlomar.out <- list(mean = z[[1L]], var = z[[2L]], ns = nns, order = order,
...
mulbar <- function (y, max.order = NULL, plot = FALSE)
{
    ...
    mulbar.out <- list(mean = z[[1L]], var = z[[2L]], v = z[[3L]],
                       aic = z[[4L]], aicmin = z[[7L]], daic=z[[5L]],
                       order.maice = z[[6L]], v.maice = z[[8L]],
                       bweight = z[[9L]], integra.bweight = z[[10L]],
                       arcoef.for = array(z[[11L]], dim = c(d, d, morder)),
                       arcoef.back = array(z[[12L]], dim = c(d, d, morder)),
                       pacoef.for = array(z[[13L]], dim = c(d, d, morder)),
                       pacoef.back = array(z[[14L]], dim = c(d, d, morder)),
                       v.bay = array(z[[15L]], dim = c(d, d)),
                       aic.bay = z[[16L]])
...
mulmar <- function (y, max.order = NULL, plot = FALSE) 
{
    ...
    for(i in 1:d)
        jnd[[i]] <- ind[(1:z[[9L]][[i]]), i]
    for(i in 1:d)
        subregcoef[[i]] <- a[(1:z[[9L]][[i]]), i]
    ...
    mulmar.out <- list(mean = z[[1L]], var = z[[2L]], v = v, aic = aic,
                       aicmin = z[[7L]], daic = daic, order.maice = z[[6L]],
                       v.maice = z[[8L]], np = z[[9L]], jnd = jnd,
                       subregcoef = subregcoef, rvar = z[[12L]], aicf = z[[13L]],
                       respns = array(z[[14L]], dim = c(d, d)),
                       matv = array(z[[16L]], dim = c(d, d)), morder = z[[18L]],
                       arcoef = array(z[[17L]], dim = c(d, d, z[[18L]])),
                       aicsum = z[[19L]])
...
perars <- function (y, ni, lag = NULL, ksw = 0)
{
    ...
    perars.out <- list(mean = z[[1L]], var = z[[2L]], subset = jnd,
                       regcoef = regcoef, rvar = z[[10L]], np = npara,
                       aic = z[[6L]], v = array(z[[8L]], dim = c(ni, ni)),
                       arcoef = array(z[[7L]], dim = c(ni, ni, z$morder)),
...
unibar <- function (y, ar.order = NULL, plot = TRUE)
{
    ...
    if (plot == TRUE) {
        oldpar <- par(no.readonly = TRUE)
        par(mfrow = c(3, 1))
        plot((0:ar.order), z[[5L]], ylim = c(0,40), type = "l", xlab = "Lag",
             ylab = "AIC(m)-aicmin (Truncated at 40.0)")
        abline(h = 0, lty = 1)
...
#        abline(h = -1/sqrt(n),lty = 3)
        ...
        for(i in 1:121)
            x[i] <- (i - 1) / 240
        plot(x, z[[17L]], type = "l", xlab = "Frequency",
             ylab = "Power Spectral Density")
        par(oldpar)
        unibar.out <- list(mean = z[[1L]], var = z[[2L]], v = z[[3L]],
                           aic = z[[4L]], aicmin = z[[7L]], order.maice = z[[6L]],
                           v.maice = z[[8L]], bweight = z[[10L]][2:(ar.order1)],
                           integra.bweight = z[[11L]], v.bay = z[[14L]],
                           aic.bay = z[[13L]], np = z[[15L]],
                           pacoef.bay = z[[12L]], arcoef = z[[16L]])
    } else {
        unibar.out <- list(mean = z[[1L]], var = z[[2L]], v = z[[3L]],
                           aic = z[[4L]], aicmin = z[[7L]], daic = z[[5L]],
                           order.maice = z[[6L]], v.maice = z[[8L]],
                           pacoef = z[[9L]], bweight = z[[10L]][2:(ar.order1)],
                           integra.bweight = z[[11L]], v.bay = z[[14L]],
                           aic.bay = z[[13L]], np = z[[15L]],
                           pacoef.bay = z[[12L]], arcoef = z[[16L]],
                           pspec = z[[17L]])
...
unimar <- function (y, max.order = NULL, plot = FALSE)
{
    ...
    if (plot == TRUE) {
        plot((0:morder), z[[5L]], ylim = c(0,40), type = "l", xlab = "Lag",
             ylab = "AIC(m)-aicmin (Truncated at 40.0)")
      abline(h = 0, lty = 1)
      unimar.out <- list(mean = z[[1L]], var = z[[2L]], v = z[[3L]],
                         aic = z[[4L]], aicmin = z[[7L]], order.maice = m,
                         v.maice = z[[8L]], arcoef = z[[9L]][1:m])
    } else { 
        unimar.out <- list(mean = z[[1L]], var = z[[2L]], v = z[[3L]],
                           aic = z[[4L]], aicmin = z[[7L]], daic = z[[5L]],
                           order.maice = m, v.maice = z[[8L]],
                           arcoef = z[[9L]][1:m])
    }
    return(unimar.out)
}
...
xsarma <- function (y, arcoefi, macoefi)
{
    ...
    xsarma.out <- list(gradi = z[[1L]], lkhoodi = z[[2L]], arcoef = coef[1:p],
                       macoef = coef[(p+1):(p+q)], grad = z[[4L]],
                       alph.ar = z[[5L]], alph.ma = z[[6L]], lkhood = z[[7L]],
                       wnoise.var = z[[8L]])
    return(xsarma.out)
}
...
decomp <- function(y, trend.order = 2, ar.order = 2, frequency = 12,
                   seasonal.order = 1, log = FALSE, trade = FALSE,
                   diff = 1, year = 1980, month = 1, miss = 0, omax = 99999.9,
                   plot = TRUE)
{
    m1 <- trend.order
    m2 <- ar.order
    ...
    if (plot == TRUE) {
        ...
        if(ilog == 0) {
            ...
            matplot(matD, pch = 0, type = "l", col = 1:2,
            ...
            for(i in 1:(n-1)) {
                ...
                y2 <- yy[i+1]
                if ((y1 != -1) && (y2 != -1))
                    lines(c(i,i+1), c(y1,y2))
                y1 <- y2
            }
            par(new=TRUE)
            plot(z[[1L]], type = 'l', col = 2, xlim = c(0, n),
                 ylim = c(ymin, ymax), main = "log(y) and Trend",
                 xlab = "", ylab = "")
        }
        ...
        if (seasonal.order != 0)
            plot(z[[2L]], type = "l", main = "Seasonal", xlab = "", ylab = "",
                 ylim = c(-my, my))
        plot(z[[5L]], type = "l", main = "Noise", xlab = "", ylab = "",
             ylim = c(-my, my))
        if (ar.order != 0)
            plot(z[[3L]], type = "l", main = "AR component", xlab = "",
                 ylab = "", ylim = c(-my, my))
        if (itrade == 1)
            plot(z[[4L]], type = "l", main = "Trading Day Effect", xlab = "",
                 ylab = "", ylim = c(-my, my))
        par(oldpar)

        decomp.out <- list(trend = z[[1L]], seasonal = z[[2L]], ar = z[[3L]],
                           trad = z[[4L]], noise = z[[5L]], aic = aic,
                           lkhd = lkhd, sigma2 = sigma2, tau1 = tau1,
                           tau2 = tau2, tau3 = tau3, arcoef = arcoef, tdf = tdf)
...
#    ifpl <- min(3*sqrt(ndata), 50, lag)
 ...
 # SPECTRUM OF IRREGULAR                               
    ...
    irregular.spec <- list(n = ndata, acov = z[[1L]], acor = z[[2L]],
                           mean = z[[3L]], v = z[[4L]], aic = z[[5L]],
                           parcor = z[[6L]], rspec = pxx, rpspec = sxx)
...
# SPECTRUM OF DIFFERENCED ADJUSTED SERIES
    ...
    adjusted.spec <- list(n = n1, acov = z[[1L]], acor = z[[2L]], mean = z[[3L]],
                          v = z[[4L]], aic = z[[5L]], parcor = z[[6L]],
...
#  PARCOR OF TREND.ORDER TIME(S) DIFFERENCED TREND SERIES
    ...
    differenced.trend <- list(n = n1, acov = z[[1L]], acor = z[[2L]],
                              mean = z[[3L]], v = z[[4L]], aic = z[[5L]],
                              parcor = z[[6L]])
...
#  PARCOR OF SEASONAL.ORDER TIME(S) DIFFERENCED SEASONAL SERIES
    ...
    differenced.season <- list(n = n1, acov = z[[1L]], acor = z[[2L]],
                               mean = z[[3L]], v = z[[4L]], aic = z[[5L]],
                               parcor = z[[6L]])
...
#        abline(h = -1/sqrt(n), lty = 3)
 ...
 to periodgram ( Order is fixed at 30 )", ylab = "Rational Spectrum", xlab = "",
 ylim = c(ymin, ymax))
        par(mfrow = c(1, 1))
...
#        abline(h = -1/sqrt(n), lty = 3)
        ...
        plot(it, type = 'l', main = "High Order AR-Spectrum as an approximation
 to periodgram( Order is fixed at 30 )", xlab = "", ylab = "Rational Spectrum",
 ylim = c(ymin, ymax))
        par(mfrow = c(1, 1))
