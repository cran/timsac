     ...
     *IDD,IR,IJ,IK,IPQ,G,A1,A,B,VD,IQM,BM,AU,ZZ,C0,AICD,TMP,
     *MJ3,MJ4,MJ6,MJ7)
C
      INCLUDE 'timsac_f.h'
C
...
c
   ...
   10 CNAME(I:I) = ' '
      ...
      IF ( I.GT.1 ) THEN
         ...
         IF (IVAR .NE. 0) THEN
            WRITE(*,*) ' ***  markov temp FILE OPEN ERROR :',CNAME,IVAR
            IFG=0
         END IF
      END IF
C
...
cc	WRITE(6,411) ISWRO
      IF (IFG.NE.0) WRITE(LU,411) ISWRO
...
cc	CALL C0GR(X,C0,G,M,L,K,ID,IAW,IPQ,MJ1,MJ2,IG)
      CALL C0GR(CYY1,MJ3,B,AW,IAW,IDD,IR,IJ,IK,X,C0,G,M,L,K,ID,IPQ,
     *          MJ4,IG,O,Q,X1,X2,CXY1,CXX1,CXV1,IFG,LU)
      DO 1500 I=1,IAW
 1500 X(I)=AW(I)
      JJ=IAW
      DO 1510 I=IDP1,K
...
cc 2181 WRITE(6,2200) I,(B(I,J),J=1,ID)
	 ...
	 WRITE(LU,340)
	 DO 1360 I=1,IAW
 1360	 WRITE(LU,351) I,AW(I)
	 WRITE(LU,53)
	 DO 2181 I=1,K
 2181	 WRITE(LU,2200) I,(B(I,J),J=1,ID)
      END IF
C
...
cc 2100 WRITE(6,2200) I,(VD(I,J),J=1,IPQ)
      IF (IFG.NE.0) THEN
	   WRITE(LU,2000)
	 DO 2100 I=1,IPQ
 2100	 WRITE(LU,2200) I,(VD(I,J),J=1,IPQ)
	 WRITE(LU,'(/)')
      END IF
C     (INVERSE OF HESSIAN) COMPUTATION
cc 2651 CALL MATINV(IPQ)
 2651 CALL MATINV(IPQ,VD,MJ4,IFG,LU)
...
cc 2600 WRITE(6,2200) I,(VD(I,J),J=1,IPQ)
	 ...
	 WRITE(LU,2500)
	 DO 2600 I=1,IPQ
 2600	 WRITE(LU,2200) I,(VD(I,J),J=1,IPQ)
      END IF
C
...
cc	CALL SUBDAV(X,C0,G,R,N,K,ID,IAW,IPQ,MJ1,MJ2,ISWRO)
      CALL SUBDAV(CYY1,MJ3,B,AW,IAW,IDD,IR,IJ,IK,VD,X,C0,G,R,N,K,ID,
     *      IPQ,AICD,MJ1,MJ4,ISWRO,O,Q,X1,X2,CXY1,CXX1,CXV1,IFG,LU)
      ISFIN=0
...
cc	SUBROUTINE C0GR(X,C0,GR,M,L,K,ID,IAW,IPQ,MJ1,MJ2,IG)
      SUBROUTINE C0GR(CYY1,MJ3,B,AW,IAW,IDD,IR,IJ,IK,X,C0,GR,M,L,K,ID,
     *                IPQ,MJ4,IG,O,Q,X1,X2,CXY1,CXX1,CXV1,IFG,LU)
...
cc	DIMENSION X(100),C0(10,10),GR(100)
      ...
      DIMENSION IDD(K),IR(K),IJ(ID),IK(ID)
...
cc	CALL NSUBX2(K,ID,KK,MJ1,MJ2,IG2)
      CALL NSUBX1(B,AW,IAW,IDD,IR,K,ID,M,IG1,X1,IFG,LU)
      CALL NSUBX2(B,AW,IAW,IDD,IR,K,ID,KK,IG2,Q,X2,IFG,LU)
      IG=IG1+IG2
...
cc	CALL GRAD(GR,M,K,ID,IAW,IPQ,MJ1,MJ2)
      CALL GRAD(IJ,IK,GR,M,K,ID,IAW,IPQ,MJ4,O,CXX1,CXV1,IFG,LU)
 1000 KSWR=1
      RETURN
      END
C
cc	SUBROUTINE NSUBX1(K,ID,M,MJ1,MJ2,IG1)
      SUBROUTINE NSUBX1(B,AW,IAW,IDD,IR,K,ID,M,IG1,X1,IFG,LU)
...
cc	WRITE( 6,2001 )
      IF (IFG.NE.0) THEN
	 WRITE( LU,2000) M,XMAX
	 WRITE( LU,2001 )
 2001	 FORMAT( 1H ,'WARNING : INCOMPLETE CONVERGENCE' )
      END IF
      IG1=1
...
C     RESCALING FOR A FEASIBLE INITIAL
      ...
      ITINP=ITINP+1
cc	CALL RESCAL(K,ID)
      CALL RESCAL(B,AW,IAW,NH,IDD,IR,IJ,IK,K,ID,IFG,LU)
cc	WRITE(6,2330) ITINP
      IF (IFG.NE.0) WRITE(LU,2330) ITINP
 2330 FORMAT(//1H ,'ITINP=',I5)
  ...
  760 M=NS-LH
cc	WRITE(6,2000) M,XMAX
      IF (IFG.NE.0) WRITE(LU,2000) M,XMAX
...
cc	SUBROUTINE NSUBX2(K,ID,KK,MJ1,MJ2,IG2)
      SUBROUTINE NSUBX2(B,AW,IAW,IDD,IR,K,ID,KK,IG2,Q,X2,IFG,LU)
...
cc	WRITE( 6,2300 )
      IF (IFG.NE.0) THEN
	 WRITE( LU,2200) KK,XMAX
	 WRITE( LU,2300 )
 2300	 FORMAT( 1H ,'WARNING : X2 INCOMPLETE CONVERGENCE' )
      END IF
      IG2=1
      RETURN
  760 KK=NS-LH
cc	WRITE(6,2200) KK,XMAX
      IF (IFG.NE.0) WRITE(LU,2200) KK,XMAX
...
cc	SUBROUTINE GRAD(GR,M,K,ID,IAW,IPQ,MJ1,MJ2)
      SUBROUTINE GRAD(IJ,IK,GR,M,K,ID,IAW,IPQ,MJ4,O,CXX1,CXV1,IFG,LU)
...
cc	CALL SUBVCP(GR,IPQ)
      IF (IFG.NE.0) WRITE(LU,1400)
      IF (IFG.NE.0) CALL SUBVCP(GR,IPQ,LU)
      RETURN
 1400 FORMAT(/1H ,'GRADIENT')
      END
C
...
cc	SUBROUTINE SUBDAV(X,C0,G,R,N,K,ID,IAW,IPQ,MJ1,MJ2,ISWRO)
      SUBROUTINE SUBDAV(CYY1,MJ3,B,AW,IAW,IDD,IR,IJ,IK,VD,X,C0,G,R,N,K,
     *    ID,IPQ,AICD,MJ1,MJ4,ISWRO,O,Q,X1,X2,CXY1,CXX1,CXV1,IFG,LU)
...
cc  150 WRITE(6,160) ITN
  150 IF (IFG.NE.0) WRITE(LU,160) ITN
...
cc  140 WRITE(6,600) ITNS,T1,EPHAI1
  140 IF (IFG.NE.0) WRITE(LU,600) ITNS,T1,EPHAI1
...
cc	CALL C0GR(SX,SC0,SG,M,L,K,ID,IAW,IPQ,MJ1,MJ2,IG)
      CALL C0GR(CYY1,MJ3,B,AW,IAW,IDD,IR,IJ,IK,SX,SC0,SG,M,L,K,ID,IPQ,
     *          MJ4,IG,O,Q,X1,X2,CXY1,CXX1,CXV1,IFG,LU)
      IF(IG.EQ.0) GO TO 215
cc	WRITE(6,211)
      IF (IFG.NE.0) WRITE(LU,211)
...
cc     AIC=',D17.5,5X,'DAIC=',D17.5)
      IF (IFG.NE.0) WRITE(LU,810) OPHAI,SPHAI,AICO,AICS,DAIC
...
cc	WRITE(6,3609) AICD
      IF (IFG.NE.0) WRITE(LU,3609) AICD
 3609 FORMAT(/1H ,'AIC=N*LOG(DET(CXX0))+2.0*IPQ=',D12.5)
...
cc 3620 WRITE(6,3630) I,(C0(I,J),J=1,ID)
	 ...
	 WRITE(LU,3610)
	 DO 3620 I=1,ID
 3620	 WRITE(LU,3630) I,(C0(I,J),J=1,ID)
      END IF
      RETURN
...
cc	SUBROUTINE  RESCAL ( K,ID )
      SUBROUTINE  RESCAL ( B,AW,IAW,NH,IDD,IR,IJ,IDUMMY,K,ID,IFG,LU )
...
cc	DIMENSION X(MJ,MJ)
   ...
   17 RETURN
      END
C
cc	SUBROUTINE SUBVCP(HH,LC)
      SUBROUTINE SUBVCP(HH,LC,LU)
C *** COMMON SUBROUTINE
C     THIS SUBROUTINE PRINTS OUT VECTOR (HH(I),I=1,LC).
      IMPLICIT REAL*8(A-H,O-Z)
      DIMENSION HH(LC)
cc	WRITE(6,39) LC
cc     WRITE(6,20)
...
cc	J2=J1+9
      J2=J1+4
      IF(J2.LE.LC) GO TO 13
   12 J2=LC
   13 JC=JC+1
cc	WRITE(6,21) I,(HH(J),J=J1,J2)
      WRITE(LU,20) (J,J=J1,J2)
      WRITE(LU,21) (HH(J),J=J1,J2)
   17 IF(J2.GE.LC) GO TO 10
cc   14 I=I+10
      GO TO 16
   10 RETURN
   39 FORMAT(/1H ,6HVECTOR,5X,3HLC=,I5)
cc   20 FORMAT(/1H ,15X,1H1,11X,1H2,11X,1H3,11X,1H4,11X,1H5,11X,1H6,11X,1
cc     AH7,11X,1H8,11X,1H9,10X,2H10)
cc   21 FORMAT(1H ,I5,4X,10D12.5)
   20 FORMAT(/5(5X,I2,9X))
   21 FORMAT(10(D12.5,4X))
      END
