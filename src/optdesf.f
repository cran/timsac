...
cc	DIMENSION P(75,75),D(75)
      DIMENSION A(M*IR,IR),B(M*IR,L),Q1(IR,IR),R(L,L)
      DIMENSION GI(L,M*IR),GIT(M*IR,IR)
      DIMENSION GL(L,L),G3(L,M*IR),GR(IR,IR),GLR(L,IR)
      DIMENSION P(M*IR,M*IR),D(M*IR)
      DIMENSION GR1(IR,IR)
...
cc  999 CONTINUE
    ...
    1 FORMAT(10I5)
    2 FORMAT(4D20.10)
   60 FORMAT(1H ,41HPROGRAM 5.5.1   OPTIMAL CONTROLLER DESIGN)
   61 FORMAT(1H ,17HINITIAL CONDITION)
   62 FORMAT(1H ,3HIR=,I5,5X,2HL=,I5,5X,3HNS=,I5)
  362 FORMAT(1H ,2HN=,I5,5X,2HM=,I5)
   63 FORMAT(//1H ,44HFIRST IR COLUMNS OF TRANSITION MATRIX (AI'S))
   64 FORMAT(//1H ,19HGAMMA MATRIX (BI'S))
   65 FORMAT(//1H ,10X,7HQ1(I,J))
   66 FORMAT(//1H ,10X,6HR(I,J))
...
cc	DIMENSION X(MJ1,MJ2),Y(MJ2,MJ1),Z(MJ1,MJ1)
      DIMENSION X(MM,NN),Y(NN,MM),Z(NN,NN)
      CST0=0.0D-00
      DO 10 I=1,MM
      DO 10 J=1,I
      SUM=CST0
...
cc	DIMENSION X(MJ1,MJ2),Y(MJ1,MJ2),Z(MJ2,MJ2)
      DIMENSION X(MM,NN),Y(MM,NN),Z(NN,NN)
      CST0=0.0D-00
      DO 10 I=1,NN
      DO 10 J=1,I
      SUM=CST0
      DO 11 K=1,MM
   11 SUM=SUM+X(K,I)*Y(K,J)
   10 Z(I,J)=SUM
      RETURN
      END
