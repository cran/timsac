...
cc      DIMENSION SXX(501)                                                
      DIMENSION Y(N)
      DIMENSION CXX(LAGH1),CN(LAGH1)
      DIMENSION A(IFPL1),B(IFPL1)
      DIMENSION PXX(LAGH1)
      DIMENSION SD(IFPL1),AIC(IFPL1),PARCOR(IFPL1-1)
C
cc      H=60                                                              
cc      H1=H+1                                                            
C                                                                       
...
cc 1031 CALL SNRASP(A,B,SXX,SGME2,L,K,H1,BMYA,BMYM)                       
 1031 CALL SNRASP(A,B,PXX,SGME2,L,K,LAGH1,PERIOD)
C
      RETURN                                                            
   50 FORMAT(1H ,'*** HIGH ORDER AR-SPECTRUM AS AN APPROXIMATION TO PERI
     *ODGRAM (ORDER IS FIXED AT 30 ) ***')                              
      END                                                               
...
cc      DIMENSION A(101),B(101)                                           
      DIMENSION A(L1),B(L1)
      DIMENSION SD1(L1),AIC1(L1),PARCOR(L1-1)
...
26101 FORMAT(1H ,////,4X,'M',11X,'INNOVATION',10X,'AIC(M)=    ',8X,     
     A      'PARTIAL AUTO-')                                            
  261 FORMAT(1H ,16X,'   VARIANCE',9X,'N*DLOG(SD(M))+2*M',              
     A      '  CORRELATION    ',                                        
     A      'PARTIAL CORRELATION (LINES SHOW +SD AND +2SD)')            
26102 FORMAT(1H+,102X,'_',7X,'_')                                       
  262 FORMAT(1H ,69X,'-1',19X,'0',19X,'1')                              
  264 FORMAT(1H ,70X,41A1)                                              
  859 FORMAT(1H ,I5,2X,2D20.5)                                          
  860 FORMAT(1H ,I5,2X,3D20.5,3X,41A1)                                  
...
cc      DIMENSION PXX(501),SXX(501)                                       
      DIMENSION A(L),B(K)                                           
      DIMENSION G(L+K+1),GR1(H1),GI1(H1),GR2(H1),GI2(H1)
      DIMENSION PXX(H1)
...
cc  702 FORMAT(6D12.5)                                                    
      RETURN                                                            
   60 FORMAT(//1H ,'RATIONAL SPECTRUM')                                 
   61 FORMAT(1H ,2HL=,I5,2X,2HK=,I5,2X,2HH=,I5)                         
   62 FORMAT(1H ,5X,'AR(I)')                                            
   63 FORMAT(1H ,5X,'MA(I)')                                            
  160 FORMAT(1H ,17HINITIAL CONDITION)                                  
  164 FORMAT(1H ,6HSGME2=,D12.5)                                        
  264 FORMAT(1H ,'AR-MA MODEL:')                                        
  265 FORMAT(1H ,'Y(N)+AR(1)Y(N-1)+...+AR(L)Y(N-L)=X(N)+MA(1)X(N-1)+...+
     AMA(K)X(N-K)')                                                     
      END                                                               
