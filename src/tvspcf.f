      SUBROUTINE TVSPCF( N, M, NOBS, NF, IVAR, SIG2, AR, VAR, SP )
C
      INCLUDE 'timsac_f.h'
C
...
C
cc      !DEC$ ATTRIBUTES DLLEXPORT :: TVSPCF
C
...
cc      DIMENSION  AR(MJ,NMAX), VNAME(5), VALUE(5), VAR(3000)
      DIMENSION  AR(M,N), VAR(N*NOBS), SP(0:NF,N)
...
cc      DIMENSION A(MJ,N), SP(0:200), YH(2000), VAR(3000), B(20)
      DIMENSION A(M,N), SP(0:NF,N), VAR(N*NOBS)
cc      ANGLE = 70.0
cc      NF = 200
C
cc      WX = 5.0
...
cc   10 SP(I) = SP(I) + DLOG10( VAR(J*NOBS-NOBS/2) )
   10 SP(I,J) = SP(I,J) + DLOG10( VAR(J*NOBS-NOBS/2) )
